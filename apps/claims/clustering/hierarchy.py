...
class HierarchyResult:
    ...
    narrative_labels: np.ndarray  # (n_points,), inherited from leaf; -1 if leaf noise
    leaf_ids: list[int]  # ordered leaf cluster ids (excludes noise)
    leaf_medoid_idxs: list[int]  # parallel to leaf_ids
    leaf_to_narrative: dict[int, int]  # leaf_id -> narrative_id (-1 if narrative noise)
...
def build_hierarchy(
    ...
    *,
    ...
    return HierarchyResult(
        ...
        n_narratives=int(narrative_result.n_clusters) if narrative_result else 0,
        n_leaf_noise=int(leaf_result.n_noise),
    )
