...
def init_lab(conn: sqlite3.Connection) -> None:
    ...
    from apps.claim_extractor.refinement_lab.meta_defaults import seed_default_meta_prompts
...
def _baseline_prompt_texts() -> tuple[str, str]:
    sys_path = _PROMPTS_DIR / "extract_system_claims_only.txt"
    usr_path = _PROMPTS_DIR / "extract_user_claims_only.txt"
    system = sys_path.read_text(encoding="utf-8-sig")
    user = usr_path.read_text(encoding="utf-8-sig")
    # Convert get_claims {{var}} to refinement lab {var}
    user = user.replace("{{text_input}}", "{text_input}").replace("{{max_claims}}", "{max_claims}")
    system = system.replace("{{max_claims}}", "{max_claims}")
    return system, user
...
def upsert_meta_prompt(conn: sqlite3.Connection, name: str, template: str) -> None:
    from apps.claim_extractor.refinement_lab.meta_defaults import validate_meta_prompt
