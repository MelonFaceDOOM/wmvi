...
TABLES_IN_ORDER = [
    # taxonomy / scrape
    ("taxonomy", "vaccine_term"),  # GENERATED ALWAYS id
    ("scrape", "job"),
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--src-prefix",
        default="PROD",
        help="Environment prefix for source DB (default: PROD)",
    )
    parser.add_argument(
        "--dst-prefix",
        default="DEV",
        help="Environment prefix for destination DB (default: DEV)",
    )
    parser.add_argument(
        "--truncate-dst",
        action="store_true",
    ...
    if dst_prefix.upper() == "PROD":
        print("Comment this check out if you want to write to PROD", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        for idx, (schema, table) in enumerate(TABLES_IN_ORDER, start=1):
            print(
            ...
            print(
                f"    -> rows in destination after copy: {rows_copied}", flush=True)
