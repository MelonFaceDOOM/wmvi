...
The YouTube transcriber downloads audio via **yt-dlp**, then runs **Whisper**. Downloads need two files under `private/` and optionally a proxy (`YT_PROXY_URL` in `.env`).
...
9. **Restart the YouTube transcriber** so it reloads the user-agent (loaded at process start in `download_yt_audio.py`):
...
| Missing formats / JS challenge errors | Outdated yt-dlp | Update yt-dlp and yt-dlp-ejs in transcription venv (`requirements-transcription.txt` installs both via `yt-dlp[default]`) |
